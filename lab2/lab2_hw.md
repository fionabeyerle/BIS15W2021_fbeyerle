...
Data matrices are series of stacked vectors, similar to a data table. 
...
```r
spring_matrix <- c(spring_1, spring_2, spring_3, spring_4, spring_5, spring_6, spring_7, spring_8)
spring_matrix
```
...
```r
temperature_matrix <- matrix(spring_matrix, nrow=8, byrow=T)
temperature_matrix
```
...
rownames(temperature_matrix) <- titles
temperature_matrix
```
...
```r
mean_vector <- rowMeans (temperature_matrix)
mean_vector
```
...
```r
temperature_matrix_m <- cbind(temperature_matrix, mean_vector)
temperature_matrix_m
```
...
```r
Jill_col <-temperature_matrix_m[ ,1]
mean(Jill_col)
```
