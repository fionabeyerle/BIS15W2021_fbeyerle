...
```r
penguins %>%
summarize(mean_bill_lengt_mm=mean(bill_length_mm, na.rm=T),
            mean_bill_depth=mean(bill_depth_mm, na.rm=T), mean_flipper_length=mean(flipper_length_mm, na.rm=T), mean_body_mass_g=mean(body_mass_g, na.rm=T),
            n=n())
```

```
## # A tibble: 1 x 5
##   mean_bill_lengt_mm mean_bill_depth mean_flipper_length mean_body_mass_g     n
##                <dbl>           <dbl>               <dbl>            <dbl> <int>
## 1               43.9            17.2                201.            4202.   344
```
